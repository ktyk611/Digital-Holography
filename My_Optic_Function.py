...
# 伝播用の関数
def Prop(image_compex:np.ndarray, z_obs ,size:int, pitch, lam):
    prop_func = H_tf_vectorized(size,pitch,z_obs,lam)
    img_fft = np.fft.fftshift(np.fft.fft2(image_compex)) * prop_func
    img_prop = np.fft.ifft2(np.fft.fftshift(img_fft))
    return img_prop

img = img_read(r"monkey.bmp")
size_y, size_x = img.shape
pitch = 20e-6
lam = 532e-9
z_obs = 0.5

img_prop = Prop(img, z_obs,size_y,pitch,lam)

plt.imshow(np.abs(img_prop),"gray")
plt.show()